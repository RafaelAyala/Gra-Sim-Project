...
void animate() {
	...
	for(j = 0; j < NUMBER_OF_BALLS; j++) {
		...
		//*******DECAY
		float p = (rand() % 101) /100; //number between 0 and 1
		if (all_spheres[j].radius > 0){
			if(p < DECAY_PROB){
				//TODO 
				//Change by how much the decay affects the ball
				//to something physically accurate?
				all_spheres[j].radius -= 0.00045;
				//printf(" Mass: %f\n" , all_spheres[j].mass);
			}
		} else {
			//printf("DONE IT SDEad LOLOLLOL");
			continue;
		}
		//printf("=========================\n");
		//************
