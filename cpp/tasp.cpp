/*
 * Tha Ayala-Stadler Project
 * 
 * This was a joint project between Rafael Ayala and Jordan Stadler to combine
 * as many elements as possible from our Simulation and Modelling course with
 * Advanced Computer Graphics course.
 *
 * The result is a transparent cube that houses a number of spheres.   
 */
 ...
 */
void update_position( Sphere *ball) {
	double a, b, c;
 ...
 * returns the position of the ball on a curve
 */
struct point3f get_position( Sphere *ball, double pos) {
	double a, b, c;
 ...
 */
void normalize_dir( Sphere *ball) {
 ...
 */
Sphere move_on_vector( Sphere ball ) {
 ...
 */
//struct sphere move_on_curve( struct sphere *ball ) {
void move_on_curve( Sphere *ball ) {
	point3f temp;
	temp = ball->pos;
 ...
 */
void update_direction( Sphere *ball ) {
	ball->direction.x = ball->pos.x - ball->previous_pos.x;
	ball->direction.y = ball->pos.y - ball->previous_pos.y;
	ball->direction.z = ball->pos.z - ball->previous_pos.z;
	normalize_dir(ball);
